...
document.addEventListener("DOMContentLoaded", function () {
  ...
  cacNumberInput.addEventListener("input", function () {
    ...
    if (!cacRegex.test(cacNumberInput.value.trim())) {
      setError(
        "cacNumber-error",
        "Invalid CAC Number (e.g., RC1234567, BN1234567, IT123456)."
      );
    } else {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    const cacNumber = cacNumberInput.value.trim() || null;
    const socialMediaHandles = document
      .getElementById("socialMediaHandles")
      .value.trim();
    const password = document.getElementById("password").value.trim();
    ...
    // Validate CAC number only if it's provided
    const cacRegex = /^(RC|BN|IT)\d{6,7}$/i;
    if (cacNumber && !cacRegex.test(cacNumber)) {
      setError(
        "cacNumber-error",
        "Invalid CAC Number (e.g., RC1234567, BN1234567, IT123456)."
      );
    ...
    try {
      const response = await fetch(
        "https://ouragent.com.ng/agent_enrollment/agentsignup.php",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
